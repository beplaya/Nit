module.exports = function Nerver(nira) {
    ...
    NERVER.CMDS = [
        { name : "DESCRIBE", action : function(option, cb){NERVER.nira.describe(option, cb)} },
        { name : "COMMENTS", action : function(option, cb){NERVER.nira.comments(option, cb)} }
    ];
        ...
        try {
            for(var i=0; i<self.CMDS.length; i++) {
                if(cmd === self.CMDS[i].name){
                    self.CMDS[i].action(option, function(data){
                        data = data || "";
