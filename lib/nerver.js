module.exports = function Nerver(nira) {
    ...
    NERVER.init = function(nit) {
        ...
        NERVER.CMDS = [
            ...
            { name : "STATUS", action : function(cb){nit.statusPrint(function(){
                    nit.logOneLiners(cb);
                })}, isNerverAction: true, runMultipleDelayed: true },
        ];
    };
    ...
    NERVER.runCommandFile = function(f) {
        ...
        try {
            var commandContent = NERVER.readSync(f) || "";
            ...
            if(command) {
                if(command.isNerverAction){
                    if(command.runMultipleDelayed) {
                        NERVER.runDelayedAction(command, NERVER.statusWatcher.statusDelay);
                        NERVER.runDelayedAction(command, NERVER.statusWatcher.redoStatusTime);
                    } else {
