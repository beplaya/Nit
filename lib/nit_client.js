module.exports = function NitClient(nerver, nettings) {
    ...
    NIT_CLIENT.sendCmdToServer = function(cmd, data, currentBranch, issueKey, tool, cb) {
        var projectKey = NIT_CLIENT.nettings.jira.projectKey;
        var dataToSend = {};
        dataToSend.data = data;
        dataToSend.currentBranch = currentBranch;
        dataToSend.issueKey = issueKey;
        NIT_CLIENT.POST(NIT_CLIENT.getOptions(NIT_CLIENT.nettings.jira.projectKey, cmd.toLowerCase(), issueKey, tool), dataToSend, function(reply){
            cb && cb(reply);
        });
    };
    ...
    NIT_CLIENT.getOptions = function(projectKey, whichData, issueKey, tool){
         return {
                    host: "localhost",
                    port: 9000,
                    path : "/rest/1.0/input/projects/"+projectKey+"/"+issueKey+"/"+whichData+"/"+tool,
