module.exports = function NitClient(nerver) {
    ...
    NIT_CLIENT.sendCmd = function(cmd, content, issueID, option, cb) {
        var self = NIT_CLIENT;
        var guid = self.generateUUID();
        NIT_CLIENT.nerver.mkdir(NIT_CLIENT.nerver.cmdDir);
        NIT_CLIENT.nerver.mkdir(NIT_CLIENT.nerver.responseDir);
        var cmdFile = self.nerver.cmdDir
            ...
            console.log(cmdFile);
        self.fs.writeFileSync(cmdFile, content);
