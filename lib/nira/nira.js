module.exports = function Nira(nettings) {
    ...
    NIRA.getSprintFineDetails = function(projectKey, sprintName, cb) {
        ...
        var statusFieldId = "";
        for(var i=0; i<NIRA.fields.length; i++) {
            if(NIRA.fields[i].name === "Status"){
                statusFieldId = NIRA.fields[i].id;
            }
        }

        var qs = require('querystring');
        var jql = NIRA.nettings.jira.issuesInSprintWithStatusJQLs.replace(/{{projectKey}}/g, projectKey);
        jql = jql.replace(/{{sprintName}}/g, sprintName);

        var statuses = ["Ready For Development",
                        "Under Development",
                        "Blocked",
                        "Pull Requested",
        ...
        NIRA._recurseStatuses(0, statusObjects, storyPointsFieldId, cb);
    };

    NIRA._recurseStatuses = function(statusIndex, statusObjects, storyPointsFieldId, cb) {
        var jiraStatus = statuses[statusIndex];
        NIRA.GET(NIRA.getOptions("search?jql=" + statusObjects[jiraStatus]),
            function(data, statusCode){
                ...
                statusIndex++;
                if(statusIndex < statusObjects.length){
                    NIRA._recurseStatuses(statusIndex, statusObjects, storyPointsFieldId, cb);
                } else {
                    cb && cb(statusObjects);
                }
            });
