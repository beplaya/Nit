module.exports = function Nira(nettings) {
    ...
    NIRA.getSprintStoryPointVelocity = function(projectKey, sprintName, cb) {
        ...
        var storyPointsFieldId = NIRA.getStoryPointsFieldId();
    ...
    NIRA.getStoryPointsFieldId = function(){
    ...
    NIRA.getSprintFineDetails = function(projectKey, sprintName, cb) {
        var qs = require('querystring');
        var storyPointsFieldId = NIRA.getStoryPointsFieldId();
        var jql = NIRA.nettings.jira.issuesInSprintWithStatusJQL;
        jql = jql.replace(/{{projectKey}}/g, projectKey);
        jql = jql.replace(/{{sprintName}}/g, sprintName);

        var statuses = NIRA.nettings.jira.statuses;

        var statusObjects = [];
        for(var i=0; i<statuses.length; i++) {
            statusObjects[statuses[i]] = {
                jql : jql.replace(/{{status}}/g, statuses[i]),
                numberOfIssues : 0,
                storyPoints : 0
            };
        }
        NIRA._recurseStatuses(0, statuses, statusObjects, storyPointsFieldId, function(statusObjects){
            var data = {
    ...
    NIRA._recurseStatuses = function(statusIndex, statuses, statusObjects, storyPointsFieldId, cb) {
        var qs = require('querystring');
        var jiraStatus = statuses[statusIndex];
        NIRA.GET(NIRA.getOptions("search?jql=" + qs.escape(statusObjects[jiraStatus].jql)),
            function(data, statusCode){
                ...
                if(statusIndex < statuses.length){
                    NIRA._recurseStatuses(statusIndex, statuses, statusObjects, storyPointsFieldId, cb);
                } else {
                    cb && cb(statusObjects);
                }
            });
